...
Matrix createLocalK(int e,mesh &m){
    ...
    //Preparando matrixH 
	deltha = m.getParameter(DELTHA);
    Ae = calculateLocalArea(e,m);
    transpose(Alpha,Alphat);
    transpose(Beta,Betat);
    productRealMatrix(deltha*Ae/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixK);

	//Preparando matrixG (En clase conocida simplemente como G)
    lambda = m.getParameter(LAMBDA);
    calculateBPrima(BPrima);
    productRealMatrix(J*lambda/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);
